---
sidebar_position: 2
title: Phase 2 — Runner Strategy Extraction
description: Introduce the Pipelines.Runner behaviour, extract the Codex turn loop into Pipelines.Codex, and turn AgentRunner into a thin dispatcher.
---

# Phase 2 — Runner Strategy Extraction

> Tracking issue: [#20](https://github.com/Mihai16/symphony/issues/20)
> Split from the [Pipeline Extension plan](https://github.com/Mihai16/symphony/blob/main/proposals/pipeline-extension-plan.md) §2 (Phase 2) and §3.4–3.7, §3.9.
> Depends on [Phase 1](./phase-1-schema-and-spec-resolver) ([#19](https://github.com/Mihai16/symphony/issues/19)).
...
- `:codex_update_recipient` — pid that receives `{:codex_worker_update, issue_id, update}` and `{:worker_runtime_info, ...}` messages. May be `nil` in tests; runners MUST treat `nil` as a no-op.
...
(lines 63–77) and emits `{:pipeline_runtime_info, issue_id, %{pipeline: name, kind: kind}}`
so the Orchestrator can populate the running-entry record before the first Codex message
arrives.
